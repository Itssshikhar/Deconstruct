...
class ActivationsDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.current_chunk_idx != chunk_idx:
            self.chunk_cache = torch.load(f'{self.temp_dir}/chunk_{chunk_idx}.pt')
    ...
    for chunk_idx in range(num_chunks):
        ...
        for i in tqdm(range(start_idx, end_idx, batch_size), 
            ...
            for example in batch['sentence']:
                with torch.no_grad():
                    activations = extract_activations(example, layer_index, model, tokenizer)
                    mean_activations = torch.mean(activations.squeeze(0), dim=0)
...
def main(resume=True):
    ...
    model_name = 'gpt2'
    gpt2_model = GPT2Model.from_pretrained(model_name, output_hidden_states=True)
    ...
    if resume:
        ...
        visualize_latent_space(latent_representations, dataset['label'])
